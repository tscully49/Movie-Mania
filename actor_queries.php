<?PHP
    ...
    $pop_directors = pg_prepare($conn, "directors", 'SELECT name as "Director Name", count(movie_id) AS "# of Movies" FROM director_of_movie AS am INNER JOIN director AS a ON (a.id = am.director_id) GROUP BY director_id, name ORDER BY count(movie_id) DESC LIMIT 20 OFFSET 1');
